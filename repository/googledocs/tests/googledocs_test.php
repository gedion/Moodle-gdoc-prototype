...
/**
  ...
  */
    private function enable_google_drive_repository() {
        ...
        $type = $this->getDataGenerator()->create_repository_type('googledocs');
        $repo = $this->getDataGenerator()->create_repository('googledocs');
        set_config('clientid', '734778225255-olpah4nhjef22mc0j9st9gh2fq0p5eu3.apps.googleusercontent.com', 'googledocs');
        set_config('secret', 'xz5EJ6VIwG-CT4yQ4_FbYkuW', 'googledocs');

        if (!$repoid = $repo->id) {
            error_log('Cannot create Googledocs repo');
        }
        return $repo;
    }
    ...
    private function create_google_user_and_enrol($course) {
        ...
        $DB->insert_record('google_refreshtokens', $userdata);
    ...
    /**
      ...
      *
      */
    public function googledocs_course_updated() {
    ...
    /**
      ...
      *
      */
    public function googledocs_course_updated_hidden() {
        $this->resetAfterTest(true);
        $course = $this->getDataGenerator()->create_course();
        $repo_instance = $this->enable_google_drive_repository();
        $course->visible = 0;
    ...
    /**
      ...
      *
      */
    public function googledocs_role_assigned() {
        $this->resetAfterTest(true);
        $course = $this->getDataGenerator()->create_course();
        $repo_instance = $this->enable_google_drive_repository();
        $sink = $this->redirectEvents();
    ...
    /**
      ...
      *
      */
    public function googledocs_role_unassigned() {
        global $DB;
        $this->resetAfterTest(true);
        $course = $this->getDataGenerator()->create_course();
        $repo_instance = $this->enable_google_drive_repository();
